...
export async function getGithubUserData() {
  const githubUserRequest = await fetch('https://api.github.com/users/robbylew')
...
export async function getGithubCommits() {
  const githubApiRequest = await fetch(
    'https://api.github.com/search/commits?q=author:robbylew'
  )
...
export async function getGithubFollowers() {
  ...
  for (let index = 1; index <= numberOfPages; index++) {
    const githubFollowersRequest = await fetch(
      `https://api.github.com/users/robbylew/followers?per_page=100&page=${index}`
    )
...
export async function getGithubRepositories() {
  ...
  for (let index = 1; index <= numberOfPages; index++) {
    const githubRepositoriesRequest = await fetch(
      `https://api.github.com/users/robbylew/repos?per_page=100&page=${index}`
    )
